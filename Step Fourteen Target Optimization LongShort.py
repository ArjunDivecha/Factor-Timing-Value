...
- T2_Final_Country_Weights.xlsx (All Periods):
  Target country weights from factor timing
...
def run_optimization(target_weights_df, returns_df, alphas_df, country_costs):
    ...
    for t, date in enumerate(common_dates):
        ...
        # Roll forward previous weights with the return earned during the
        # holding month t-1 -> t. Returns use the FORWARD convention (Step One:
        # row t = return earned from t to t+1), so the t-1 -> t return is
        # row t-1. Using row t here would inject look-ahead.
        if t > 0:
            holding_period_returns = returns_aligned.iloc[t-1]
            rolled_forward_weights = roll_forward_weights(previous_weights, holding_period_returns)
        else:
...
def calculate_portfolio_returns(weights_df, returns_df):
    ...
    Timing (matches fixed Step Nine): returns use the FORWARD convention
    (Step One builds 1MRet with shift(-1)), so row t of returns_df already
    holds the return earned from month t to t+1. Month-t weights are
    therefore applied to returns row t — no extra shift.
    ...
    Returns:
        pandas Series: Portfolio returns (indexed by weight date t,
        covering the holding month t -> t+1)
    """
    # Get weight dates
    ...
    for i, date in enumerate(weight_dates):
        ...
        returns = returns_df.loc[date]  # forward return t -> t+1
        
        # Skip stub months with no realized forward returns yet
        if returns.isna().all():
            continue
    ...
    with pd.ExcelWriter(output_file, engine='xlsxwriter') as writer:
        ...
        config_df = pd.DataFrame({
            'Parameter': ['Alpha_Mult', 'Lambda_Drift_Penalty', 'Transaction_Cost',
                          'Country_Cost_Shape_Mult', 'Max_Weight'],
...
def main():
    ...
    logging.info(f"Max Weight per country: {MAX_WEIGHT:.1%}")
    logging.info("="*80)
    ...
    try:
        ...
        # Load country trading costs
        common_countries = target_weights_aligned.columns.tolist()
        country_costs = load_country_trading_costs(common_countries, country_costs_df)
